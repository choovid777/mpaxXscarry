...
class _BugSenderPageState extends State<BugSenderPage>
  ...
  Future<void> _addSender(String number, bool isGlobal) async {
    ...
    try {
      final res = await http.get(
        Uri.parse(
          "http://188.166.180.7:3000/getPairing?key=${widget.sessionKey}&number=$number&global=${isGlobal ? 1 : 0}",
        ),
      );
      final data = jsonDecode(res.body);
      if (res.statusCode == 200 && data["valid"] == true) {
  ...
  Future<void> _deleteSender(String id, bool isGlobal) async {
    ...
    try {
      final res = await http.delete(
        Uri.parse(
          "http://188.166.180.7:3000/deleteSender?key=${widget.sessionKey}&id=$id&scope=${isGlobal ? 'global' : 'private'}",
        ),
      );
      final data = jsonDecode(res.body);
      if (res.statusCode == 200 && data["valid"] == true) {
  ...
  void _showSnackBar(String msg, bool isError) {
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        ...
        backgroundColor: isError ? Colors.red.withOpacity(0.9) : accentRed.withOpacity(0.9),
                        ...
                        child: Row(
                          ...
                          children: [
                            Icon(
                              ...
                              color: canDelete ? Colors.redAccent : softGrey,
                              size: 18,
                            ),
                            const SizedBox(width: 8),
                            Text(
